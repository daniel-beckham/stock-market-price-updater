...
from website.routes import general_blueprint, stock_data_blueprint, stock_info_blueprint
...
def create_app():
    ...
    app = Flask(
        __name__,
        static_url_path=getenv("SUBDIRECTORY", "") + "/static",
    ...
    if getenv("GOOGLE_CLOUD_PROJECT", "") != "":
        ...
        app.config["SQLALCHEMY_DATABASE_URI"] = (
            ...
            "{password}@/{database}?host=/cloudsql/{instance}"
        ).format(**postgres)
    else:
    ...
    app.register_blueprint(general_blueprint, url_prefix=blueprint_subdirectory)
    app.register_blueprint(
        stock_data_blueprint, url_prefix=blueprint_subdirectory + "/api/data"
    )
    app.register_blueprint(
        stock_info_blueprint, url_prefix=blueprint_subdirectory + "/api/info"
    )
